...
const DayAppointmentContainer = () => {
  ...
  useEffect(() => {
    if (selectedDate) {
      const from = moment(selectedDate).startOf("dates").valueOf();
      const to = moment(selectedDate).endOf("dates").valueOf();
  ...
  const handleSelectedDateChange = (action: CHANGE_DATE_ACTION_ENUM) => {
    return () => {
      ...
      if (action === CHANGE_DATE_ACTION_ENUM.INCREASE) {
        setSelectedDate(moment(selectedDate).add(1, "days").valueOf());
        calendarApi.next();
      } else {
        setSelectedDate(moment(selectedDate).subtract(1, "days").valueOf());
        calendarApi.prev();
      }
    };
  };
...
export default DayAppointmentContainer;
