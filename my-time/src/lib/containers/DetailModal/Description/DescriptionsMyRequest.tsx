...
const DescriptionsMyRequest: FC<IDescription> = ({ url }) => {
      ...
      <StyledDescription>
        <Spin spinning={isLoadingEscalate || isLoadingData}>
          <Detail>
            ...
            {typeDes !== 'leaves' && (
              <DetailRow>
                ...
                <DetailValue>
                  ...
                  moment(data?.data?.toDate).format('l').toString()
                    ? moment(
                        data?.data?.fromDate ? data?.data?.fromDate : data?.data?.createdDate
                      ).format('l')
                    : moment(data?.data?.fromDate).format('l') +
                      ' - ' +
                      moment(data?.data?.toDate).format('l')}
            ...
            {['ot-requests', 'timesheet-updates'].includes(typeDes) && (
              <DetailRow>
                <DetailLabel>{t('modal.duration')}:</DetailLabel>
                <DetailValue>
                  {typeDes === 'timesheet-updates'
                    ? data?.data?.updateTotalTime
                      ? Number(data?.data?.updateTotalTime) / 60 / 60 + 'h'
                      : '-'
                    : data?.data?.duration
                    ? Number(data?.data?.duration) / 60 / 60 + 'h'
                    : '-'}
                </DetailValue>
              </DetailRow>
            )}
...
export default DescriptionsMyRequest;
