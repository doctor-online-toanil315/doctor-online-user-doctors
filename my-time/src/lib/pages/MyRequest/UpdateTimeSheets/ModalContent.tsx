...
import { usePutUpdateTimeSheetsMutation } from '../../../services';
...
const ModalContent: FC<ModalContent> = ({ handleCloseModal, record, id }) => {
  ...
  const [putUpdateTimeSheets, { isLoading }] = usePutUpdateTimeSheetsMutation();

  const handleSubmit = () => {
    putUpdateTimeSheets({ body: id, id: record.key })
      .unwrap()
      .then(() => {
