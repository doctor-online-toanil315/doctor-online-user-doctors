...
import { usePutWorkingAfterHourssMutation } from '../../../services';
...
const ModalContent: FC<ModalContent> = ({ handleCloseModal, record, id }) => {
  ...
  const [putWorkingAfterHourss, { isLoading }] = usePutWorkingAfterHourssMutation();

  const handleSubmit = () => {
    putWorkingAfterHourss({ body: id, id: record.key })
      .unwrap()
      .then(() => {
