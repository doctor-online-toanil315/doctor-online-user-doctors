...
import { StyledBoxShadow, StyledTable, StyledTimePanel, StyledViewDetail } from './styles';
...
const TimePanel = ({ dataWorkingHour }: TimeProps) => {
  ...
  const [isViewDetail] = usePermission([permissionOverviewEnum.VIEW_WORKING_HOUR_DETAIL]);
  return (
    <StyledTimePanel>
      <StyledTable>
        ...
        <div className="table-row-group">
          ...
          <div className="table-cell border">
            {dataWorkingHour?.data?.items.length > 0 && dataWorkingHour?.data?.items[1].inTime
              ? formatDate(dataWorkingHour?.data?.items[1].inTime * 1000, 'HH:mm', true)
              : '-:-'}
          </div>
          <div className="table-cell">
            {dataWorkingHour?.data?.items.length > 0 && dataWorkingHour?.data?.items[1].outTime
              ? formatDate(dataWorkingHour?.data?.items[1].outTime * 1000, 'HH:mm', true)
              : '-:-'}
          </div>
        </div>
      </StyledTable>
